...
use super::{
    ...
    CardModifierValue, CardPile, LIVE_CARD_MAX_EFFECT_OVERRIDES, LIVE_CARD_MAX_ID_BYTES,
    LIVE_CARD_MAX_MODIFIERS, LIVE_CARD_MAX_TEXT_BYTES, LiveCardCollection, LiveCardError,
    LiveCardField, LiveCardFixture, LiveCardProjection, LiveCardSnapshot, LiveCardValue,
};
...
fn validate_projection(projection: &LiveCardProjection) -> Result<(), LiveCardError> {
    ...
    for key in projection.effect_parameter_overrides.keys() {
...
fn validate_local_key(value: &str, field: &'static str) -> Result<(), LiveCardError> {
...
fn validate_text(value: &str, field: &'static str) -> Result<(), LiveCardError> {
