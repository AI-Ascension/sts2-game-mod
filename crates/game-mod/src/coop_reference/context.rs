...
impl CoopPartyContext {
    /// Returns whether this context states no step, vote or targeting at all.
    ///
    /// A declared-unavailable family carries no party record, and a context that states a step or
    /// a vote is a party in its own right, so it is refused rather than dropped from the snapshot.
    #[must_use]
    pub fn is_unstated(&self) -> bool {
        self.step == 0 && !self.votes.is_present() && !self.targeting.is_present()
    }
}
