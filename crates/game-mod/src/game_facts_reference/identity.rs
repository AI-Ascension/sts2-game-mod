...
//! Opaque identity acceptance for rule ids, input names, units and build tokens.

use super::GameFactsError;
use super::model::{GAME_FACTS_MAX_IDENTITY_BYTES, GAME_FACTS_MAX_IDENTITY_SEGMENTS};
...
/// and free of the traversal segments that would make it addressable on a host filesystem. An
/// identity that could be read as a path is refused so this vocabulary can never be turned into a
/// host read.
#[must_use]
pub fn is_opaque_facts_identity(value: &str) -> bool {
    !value.is_empty()
        && value.len() <= GAME_FACTS_MAX_IDENTITY_BYTES
        ...
        && value.split('.').count() <= GAME_FACTS_MAX_IDENTITY_SEGMENTS
}
...
pub(super) fn validate_opaque_identity(
    ...
    if is_opaque_facts_identity(value) {
        Ok(())
    } else {
        Err(GameFactsError::NonOpaqueIdentity(field))
    }
}
