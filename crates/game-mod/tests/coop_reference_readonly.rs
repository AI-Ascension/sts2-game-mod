...
#[test]
fn a_live_fence_naming_another_epoch_is_refused_with_both_epochs() {
    let (_manifest, catalog) = fixture_catalog();
    let scope = CoopReadScope::Party;
    assert_eq!(
        catalog
        ...
        "a fence from another observation epoch is refused rather than resolved against the held one"
    );
}

#[test]
fn a_reference_naming_a_membership_generation_the_party_no_longer_has_is_refused() {
    let (_manifest, catalog) = fixture_catalog();
    let mut reference = peer_reference(&catalog, "peer.ally");
    assert!(
        catalog.peer(&reference, CoopReadScope::Peer).is_ok(),
    ...
    assert_eq!(
        catalog
            .peer(&reference, CoopReadScope::Peer)
            .expect_err("pre-rejoin reference"),
        CoopError::StalePeerGeneration("peer.ally".to_owned()),
        "a reference minted before a rejoin names a member that no longer exists"
    );
}
