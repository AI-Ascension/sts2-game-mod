...
use manifest_fixture::{fixture_manifest, manifest, quantity};
use sts2_game_mod::{
    CoopEffectScope, CoopEntryKind, CoopError, CoopFamilyCoverage, CoopFamilyState, CoopFieldValue,
    CoopPartyInput, CoopPeerInput, CoopPeerRole, CoopPileKind, CoopPileView, CoopReadScope,
    CoopScalingKind,
};
use support::*;
...
fn a_collection_past_its_local_bound_is_refused() {
    ...
    snapshot.party.peers = (0..=sts2_game_mod::COOP_MAX_PEERS)
        .map(|index| {
            ...
            peer.generation = index as u64 + 1;
            if index > 0 {
                peer.role = CoopPeerRole::Ally;
            }
            peer
...
fn a_party_past_its_aggregate_byte_bound_is_refused_with_the_measured_size() {
    ...
    let error = produce_snapshot(snapshot).expect_err("too large");
    assert!(
        matches!(
            &error,
            CoopError::PartyTooLarge { limit, actual }
                if *limit == sts2_game_mod::COOP_MAX_PARTY_BYTES && *actual > *limit
        ),
        "expected a measured byte refusal past the bound, got {error:?}"
    );
}
