...
pub fn ally_peer() -> CoopPeerInput {
    CoopPeerInput {
        ...
        readiness: CoopFieldValue::present(false),
        piles: ally_piles(),
    }
}

/// The public context of the fixture party: one open vote and one targeting relationship.
///
/// The vote names the members that have signalled a choice but not the choice any of them made,
/// because a member's own pick is its private state.
pub fn fixture_context() -> CoopPartyContext {
    CoopPartyContext {
        phase: CoopTurnPhase::Selection,
        step: 3,
        votes: CoopFieldValue::present(vec![CoopVote {
            vote_id: "vote.reward".to_owned(),
            phase: CoopTurnPhase::Selection,
            choices: CoopFieldValue::present(vec![
                CoopVoteChoice {
                    choice_id: "choice.card".to_owned(),
                    label: CoopFieldValue::present("Take the card".to_owned()),
                },
                CoopVoteChoice {
                    choice_id: "choice.skip".to_owned(),
                    label: CoopFieldValue::absent(),
                },
            ]),
            decided_peer_ids: vec!["peer.local".to_owned()],
            resolved: false,
        }]),
...
pub fn peer_reference(catalog: &CoopCatalog, peer_id: &str) -> CoopPeerReference {
    let generation = catalog
        .party()
        .map(|record| record.peer_reference(peer_id).generation)
        .unwrap_or_else(|_| generation_of(peer_id));
    CoopPeerReference {
        catalog: catalog.binding().clone(),
        party_id: "party.alpha".to_owned(),
...
/// The membership generation of one fixture member, so a reference names the member it means.
///
/// A rejoin increments a member's generation, so a reference minted before one names a member that
/// no longer exists; the fixture derives the current generation rather than hard-coding it.
pub fn generation_of(peer_id: &str) -> u64 {
    fixture_party()
        .peers
        .iter()
...
/// The observation epoch the fixture party was read at.
///
/// A live fence names the epoch of the instance's observation stream, so a fence from another
/// episode is refused rather than answered with this party's record.
pub fn fixture_epoch() -> u64 {
    fixture_party().epoch
}
