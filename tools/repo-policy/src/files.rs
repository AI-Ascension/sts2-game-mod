...
pub(crate) fn required_findings(root: &Path, policy: &Policy) -> Vec<Finding> {
    policy
        .required_files
        .iter()
        ...
        .filter_map(|(relative, reason)| {
            ...
            if path.is_absolute() || path.components().any(|part| part == Component::ParentDir) {
                Some(Finding::error(
                    "EXC001",
                    relative,
                    "exemption must be repository-relative",
                ))
    ...
    files
        ...
        .filter_map(|path| {
            ...
            let is_python = matches!(
                ...
                Some("py" | "pyi" | "pyc" | "pyo")
            ) || PYTHON_NAMES.contains(&name)
                || (name.starts_with("requirements")
                    && extension.is_some_and(|value| value.eq_ignore_ascii_case("txt")));
            is_python.then(|| {
                Finding::error(
                    "LANG001",
                    &relative_text(root, path),
                    "Python source or package metadata is prohibited",
                )
            })
        })
        .collect()
...
fn size_category(relative: &Path) -> Option<SizeCategory> {
    ...
    match relative.extension().and_then(|value| value.to_str()) {
        Some("rs") if is_test_path(relative, &name) => Some(SizeCategory::RustTest),
        Some("rs") => Some(SizeCategory::RustProduction),
        Some("cs") if is_test_path(relative, &name) => Some(SizeCategory::CsharpTest),
...
fn is_test_path(path: &Path, name: &str) -> bool {
...
mod tests {
    ...
    use super::size_category;
    use crate::config::SizeCategory;

    #[test]
    fn classifies_source_and_test_paths() {
        assert_eq!(
            size_category(Path::new("crates/host/src/lib.rs")),
            Some(SizeCategory::RustProduction)
        );
        assert_eq!(
            size_category(Path::new("crates/host/tests/queue.rs")),
            Some(SizeCategory::RustTest)
        );
        assert_eq!(
            size_category(Path::new("managed/loader/Entry.cs")),
