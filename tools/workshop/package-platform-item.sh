...
usage() {
    printf 'usage: %s <platform> <payload-dir> <output-dir> <consumer-app-id> <published-file-id> <game-version> <package-version> <source-revision> <preview-file>\n' "$SCRIPT_NAME" >&2
...
case "$platform" in
    windows-x86_64)
        native_payload='AIAscensionSTS2GameModNative.dll'
        ;;
    linux-x86_64)
        native_payload='libAIAscensionSTS2GameModNative.so'
        ;;
    *)
        die "unsupported platform: $platform (expected windows-x86_64 or linux-x86_64)"
        ;;
esac

payload_dir=$(realpath -e "$payload_arg") || die "payload directory does not exist: $payload_arg"
output_dir=$(realpath -m "$output_arg") || die "could not resolve output directory: $output_arg"
[[ ! -L "$payload_arg" && ! -L "$preview_arg" ]] || die 'payload root and preview must not be symlinks'
preview_file=$(realpath -e "$preview_arg") || die "preview file does not exist: $preview_arg"
...
canonical_record "$native_payload"
...
manifest_path="$output_dir/$MANIFEST_NAME"
{
    ...
    printf '    {"path": "%s", "role": "native_library", "size_bytes": %s, "sha256": "%s"}\n' "$native_payload" "$(file_size "$output_dir/$native_payload")" "$(file_digest "$output_dir/$native_payload")"
    printf '  ],\n'
