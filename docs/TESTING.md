...
Run the [required enemy observation probe](../experiments/managed-rust-interop/enemy-projection-tests/README.md) with the pinned .NET SDK; it links production code with host doubles and does not establish native support.
...
For bounded native Astra treasure opening, relic acquisition and map continuation, see [the treasure evidence](evidence/native-treasure-20260906.md). Shop entry remained unresolved.
...
The managed host-thread capture seam is the witness for the host-side half of that owner boundary: the synthetic-host probe `experiments/managed-rust-interop/checkpoint-capture-tests/CheckpointCaptureProbe.csproj` links the host-free production sources only, drives the seam through a stub reader, and checks that an off-host-thread call is refused before any host read, that mid-effect and enemy-execution settlements are refused with their typed details, that a pending selection transition is refused, that a quiescent capture is structurally identical to the pinned valid fixture and its bytes hash to that vector's pinned canonical blob digest, that mutating host state after capture changes neither the record nor the bytes and no synthetic host object is reachable from the outcome graph, that two captures leave the observation generation and RNG draw count untouched, and that a required `unknown` family, a misplaced combat family, an outstanding pending effect, an unbounded collection, a duplicate RNG stream id, a malformed identifier and a throwing host read are each refused with their exact typed detail. Hosted `managed-source` CI runs it; it needs no game assembly and proves nothing about native behavior.
...
It keeps zero and empty values available, records distinct not-applicable/not-observed/unsupported/denied/unknown/failed outcomes, fences detail recovery by manifest/snapshot/epoch, rejects unknown or protected field paths, reports count-known and required-field coverage, traverses bounded collections with opaque continuations, and returns a typed oversized-detail error. The synthetic fixture is not a protocol artifact, gateway route, MCP tool, or native host compatibility claim; those remain separately negotiated and verified.

## Owner-local content index fixture

The manifest fixtures (`content_manifest` and `content_manifest_provenance`) require independent registry totals, distinguish verified zero from missing evidence, and reject missing/extra count families and under/over-extraction for handled and unhandled families. They also verify canonical input ordering, package removal, semantic identity across adapter support changes, locale/text separation, and inventory/cursor invalidation. These synthetic counts do not validate a host registry. Exact-build, mode, registry API, supported-family and exclusion evidence remains required.

The source-only content-index producer binds typed names, aliases, descriptions, package provenance, filters, unlock observations, and exact-detail capabilities to one manifest; its fixture covers unsupported-family registry rebasing, bounded pagination/detail payloads, source identity/alias validation, literal non-ASCII search ranking, duplicate names, capability/source consistency, exact lookup errors, locked-reference scope, stale cursors, locale fencing, and source failures (run `cargo test --locked --offline --package sts2-game-mod --test content_index --test content_index_regressions`). The index is immutable after production; only opaque reader cursors are mutable. Unknown or unsupported source data fails closed; no fixture constructs playable objects, mutates profile state, or asserts native compatibility. Names/errors are game-mod-local pending the separately owned versioned contract.

Owner-local character reference fixture: the source-only producer binds manifest/locale-fenced character IDs, localized text/provenance, generic starting HP/resources/gold/potion capacity, starting deck/relic references, pool/mechanic/loadout prerequisites, progression unlock requirements, and explicit formula/unavailable values; field wrappers distinguish unavailable collections from observed empty collections, and the continuation type is non-clonable/single-use. `cargo test --locked --offline --package sts2-game-mod --test characters` covers paging, visibility, cursor/reference/locale/manifest fences, source failures, unknown references, duplicates, oversized input, and read-only reads. Synthetic source/build evidence only; no active-run/setup projection, settled-run confirmation, wire route, MCP/gateway delivery, profile mutation, or native compatibility is claimed.
