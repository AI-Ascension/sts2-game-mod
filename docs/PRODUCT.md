...
The host-independent core target owns semantic policy. The accepted sts2-protocol target owns
shared language- and transport-neutral contracts only after its own decision and conformance work.
This target owns no MCP tool catalog, gateway lease, model schema, or experiment artifact.
...
versioned ABI validation, a transport-free bounded HTTP adapter, and their composition. Deterministic
fake tests cover those seams. This remains source-only preparation: it adds no game rules, public
route catalog, managed loader, host callback implementation, game package, or placeholder crate.
...
required, and reconciled with observed host state. The current ports do not make those runtime
claims.
