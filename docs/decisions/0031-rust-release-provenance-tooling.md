# ADR 0031: Rust release provenance tooling boundary
...
manifest, and platform-specific Workshop staging. Repository policy prohibits Python production
source, while the historical shell entry points are used by operators and existing checks.

## Decision

The `sts2-release-tool` workspace member owns the three release commands:
