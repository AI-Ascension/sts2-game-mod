...
internal sealed partial class InstalledNativeCoopHostPort
{
    ...
    public CoopEffectWitness? Reconcile(string operationId)
    {
        ...
        // RunManager's ordinary post-action callback and the semantic synchronizer callbacks are
        // the supported sources for this checkpoint. They emit the same checkpoint on every
        // peer; calling GenerateChecksum here would create an extra unsynchronised checkpoint.
            ...
            after.HostStateDigest)
        {
            ...
            NativeChecksum = after.NativeChecksum
        };
    ...
    public void ConfirmSettlement(string operationId)
    {
        if (_pending.TryGetValue(operationId, out NativePendingOperation? pending))
        {
            ...
            // CoopHostRuntime retains the settled receipt for idempotency. The native handle
            // is no longer needed after the outer runtime has accepted the fresh all-peer
            // witness, so remove it to keep the adapter's bounded pending set available for
