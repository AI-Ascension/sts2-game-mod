    ...
    string StateDigest)
{
    ...
    // The native checksum is the host-backed checkpoint witness when the installed service
    // exposes it. It remains nullable for adapters that can prove a state transition only after
    // the checksum message is read.
...
internal interface ICoopNativeHostPort
{
    ...
    // The native adapter may retain an operation-local completion marker, but it may only mark
    // that operation complete after the outer runtime has accepted the fresh all-peer
    // convergence observation. Test ports do not need to retain such a marker.
    void ConfirmSettlement(string operationId) { }
    ...
    CoopNativeLegalCatalog LegalCatalog(CoopHostObservation observation) =>
        CoopNativeLegalCatalog.Empty(observation);
