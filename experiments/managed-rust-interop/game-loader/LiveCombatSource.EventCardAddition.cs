...
{
    private bool PrepareEventCardAddition(LegalActionReference action, out Func<Task> invoke,
        out Func<bool> postcondition, out string effect)
    {
        invoke = () => Task.CompletedTask;
        postcondition = () => false;
        effect = "";
        if (action.Kind != "select_card" || RewardOverlay() is not NSimpleCardSelectScreen screen
        ...
        invoke = async () =>
        {
            ...
            throw new InvalidOperationException("native event card selection did not close");
        };
        ...
        effect = "event_card_added_to_deck";
        return true;
    }
}
