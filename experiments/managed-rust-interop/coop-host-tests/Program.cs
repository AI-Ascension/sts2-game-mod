...
internal static partial class Program
{
    ...
    private static void Main()
    {
        ...
        AuthorityChangePrunesPendingReceipt();
    ...
    private static void NativeArgumentEncodingFailsClosed()
    {
        ...
        CoopSharedVoteRequest mapRequest = new(
            "op:map", "map:1", 1, "peer:host1", CoopVoteDomain.Map, "map:1:4:2");
        Check(NativeVoteEncoding.TryParse(mapRequest, out NativeVoteEncoding mapVote, out _)
            && mapVote.MapActIndex == 1 && mapVote.MapRow == 4 && mapVote.MapColumn == 2
            && mapVote.MapGenerationCount < 0,
            "map vote maps to an active-act coordinate and current map generation");

        CoopSharedVoteRequest playerChoiceRequest = new(
            "op:choice", "choice:1", 1, "peer:host1", CoopVoteDomain.PlayerChoice,
            "indexes:0,2");
        Check(NativeVoteEncoding.TryParse(playerChoiceRequest,
                out NativeVoteEncoding playerChoiceVote, out _)
            && playerChoiceVote.PlayerChoice?.Kind == NativePlayerChoiceKind.Indexes
            && playerChoiceVote.PlayerChoice.Indexes is { Count: 2 } indexes
            && indexes[0] == 0 && indexes[1] == 2,
            "player choice vote maps to a bounded native index list");

        CoopSharedVoteRequest skippedPlayerChoiceRequest = new(
            "op:choice-skip", "choice:1", 1, "peer:host1", CoopVoteDomain.PlayerChoice,
            "skip");
        Check(NativeVoteEncoding.TryParse(skippedPlayerChoiceRequest,
                out NativeVoteEncoding skippedPlayerChoiceVote, out _)
            && skippedPlayerChoiceVote.PlayerChoice?.Kind == NativePlayerChoiceKind.Index
            && skippedPlayerChoiceVote.PlayerChoice.Index is null,
            "player choice skip maps to a nullable native index");
